...
function EntradaVeiculo() {
    ...
    const { adicionarCliente, buscarClientes, buscarClientePorPlaca } = Clientes()
    const { buscarVeiculosPorCliente } = Veiculos()
    ...
    const buscarVeiculosEDefinir = async (idCliente) => {
        var veiculos = await buscarVeiculosPorCliente(idCliente);
        console.log(veiculos);
        setVeiculosDisponiveis(veiculos);
    }

    const buscarClientesEDefinir = async () => {
    ...
    useEffect(() => {
        const carregarDadosPorPlaca = async () => {
            ...
            try {
                ...
                const veiculoPayload = veiculoResponse?.data;
                const clientePayload = clienteResponse;

                const veiculo = Array.isArray(veiculoPayload)
                    ? (veiculoPayload[0] || null)
                    : (veiculoPayload || null);

                const cliente = Array.isArray(clientePayload)
                    ? (clientePayload[0] || null)
                    : (clientePayload || null);
                ...
                setDadosEntradaExistente((prev) => ({
                    ...prev,
                    idCliente: veiculo?.id_cliente || veiculo?.idCliente || cliente?.id_cliente || cliente?.idCliente || prev.idCliente,
                    idVeiculo: veiculo?.id_veiculo || veiculo?.idVeiculo || prev.idVeiculo,
                    idOrdemServico: null,
                    idRegistroEntrada: null
                }));
            } catch (error) {
    ...
    useEffect(() => {
        ...
        buscarVeiculosEDefinir(clienteSelecionado.id_cliente);
    }, [clienteSelecionado]);
    ...
    useEffect(() => {
        if (formData.tipo_cliente !== "PESSOA_FISICA" && preencherComDadosCliente) {
            setPreencherComDadosCliente(false);
        }
    }, [formData.tipo_cliente, preencherComDadosCliente]);

    useEffect(() => {
        if (!preencherComDadosCliente || formData.tipo_cliente !== "PESSOA_FISICA") return;
    ...
    }, [preencherComDadosCliente, formData.tipo_cliente, formData.nome, formData.cpf_cnpj]);
                ...
                <div className="coluna-esquerda">
                    <InformacoesCard titulo="Informações do Cliente" icone="bx bx-user">
                        ...
                        <div className="input-field">
                            ...
                            <div>
                                <input type="radio" name="tipo_cliente" value="PESSOA_FISICA" checked={formData.tipo_cliente === "PESSOA_FISICA"} onChange={handleChange} disabled={camposBaseBloqueados || clienteBloqueadoPorSelecao} />
                                <label>Pessoa Física</label>
                                <input type="radio" name="tipo_cliente" value="PESSOA_JURIDICA" checked={formData.tipo_cliente === "PESSOA_JURIDICA"} onChange={handleChange} disabled={camposBaseBloqueados || clienteBloqueadoPorSelecao} />
